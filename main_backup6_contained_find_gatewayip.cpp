...
void change_sender_arp_table(pcap_t* handle,const Mac& my_mac, const Ip& target_ip, const Ip& sender_ip){
	...
	while (1) {
		if(flag_send){//flag == 0: 수신만 기능
			if(flag_send==1){//get sender, target's mac address
				...
				printf("Attack!!\n");
				change_sender_arp_table(handle,my_mac, target_ip, sender_ip);
